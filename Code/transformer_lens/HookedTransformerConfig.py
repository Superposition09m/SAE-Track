...
class HookedTransformerConfig:
    ...
    Args:
        ...
        final_rms (bool): Whether to replace the final normalization (just
            ...
            scaling + weights). Only included because of a dumb bug in
        ...
        n_params (int, *optional*): The number of (hidden weight)
            ...
            the [scaling laws paper](XXXX) found
    ...
    def convert_to_dict(self) -> dict:
        """
        Convert the HookedTransformerConfig instance to a dictionary, handling non-serializable types.
        """
        cfg_dict = {
            **self.__dict__,
            "device": str(self.device),  # Convert device to string
            "dtype": str(self.dtype),  # Convert dtype to string
        }
        return cfg_dict

    @classmethod
    def convert_from_dict(cls, cfg_dict: dict) -> HookedTransformerConfig:
        """
        Convert a dictionary to a HookedTransformerConfig instance, handling deserialization of specific types.
        """
        # Handle special cases for deserialization
        cfg_dict['device'] = device(cfg_dict['device'])  # Convert string back to device
        cfg_dict['dtype'] = {"torch.float16": torch.float16, "torch.float32": torch.float32, "torch.float64": torch.float64}[cfg_dict['dtype']]  # Convert string back to dtype
        return cls(**cfg_dict)
