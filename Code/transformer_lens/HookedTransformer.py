...
class HookedTransformer(HookedRootModule):
    ...
    def to_tokens(
        ...
        token, others (OPT and models) were)
    ...
    def init_weights(self):
        ...
        no one has gotten round to updating it?
        ...
        integrated those ideas yet: XXXX
    ...
    def fold_layer_norm(
        ...
        if not self.cfg.final_rms and fold_biases:
            # Dumb bug from old SoLU training code, some models have RMSNorm instead of LayerNorm
            # pre unembed.
    ...
    def refactor_factored_attn_matrices(self, state_dict: Dict[str, torch.Tensor]):
        ...
        Circuits](XXXX), queries, keys and
    ...
    def save(self, save_path: str):
        """
        Save the HookedTransformer model configuration and state_dict to the specified path.

        Args:
            save_path (str): The directory where the model and configuration will be saved.
        """
        # Create the directory if it doesn't exist
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        elif os.path.isdir(save_path) and os.listdir(save_path):
            warnings.warn(f"The directory {save_path} already exists and is not empty. Existing files may be overwritten.", UserWarning)

        # Save the model configuration
        config_path = os.path.join(save_path, "config.json")
        with open(config_path, "w") as f:
            json.dump(self.cfg.convert_to_dict(), f)

        # Save the model state dict
        model_state_path = os.path.join(save_path, "model_state.pt")
    ...
    def load(cls, load_path: str) :
        """
        Load the HookedTransformer model configuration and state_dict from the specified path.

        Args:
            load_path (str): The directory where the model and configuration are saved.
        ...
        cfg = HookedTransformerConfig.convert_from_dict(cfg_dict)

        # Initialize the model
        model = cls(cfg)

        # Load the model state dict
        model_state_path = os.path.join(load_path, "model_state.pt")
        model.load_state_dict(torch.load(model_state_path, map_location=cfg.device))

        # Load the tokenizer, if it exists
        tokenizer_path = os.path.join(load_path, "tokenizer_config.json")
