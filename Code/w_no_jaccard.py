...
def calculate_non_self_similarity(activations, top_k):
    # Binarize activations: Convert to 1 where non-zero, else 0
    binary_activations = (activations[:top_k] != 0).float()  # Shape: (top_k, feature_dim)

    # Compute intersection and union using broadcasting
    intersection = torch.matmul(binary_activations, binary_activations.T)  # Shape: (top_k, top_k)
    union = binary_activations.sum(dim=1, keepdim=True) + binary_activations.sum(dim=1) - intersection

    # Compute Jaccard Similarity
    jaccard_matrix = intersection / (union + 1e-8)  # Add epsilon to avoid division by zero

    # Mask the diagonal (self-to-self similarities)
    mask = torch.eye(top_k, dtype=torch.bool, device=activations.device)  # Diagonal mask
...
plt.savefig(f"jaccard_similarity_diff_{model_type}_{hkpt}_{random_save_id}.svg")
plt.show()
