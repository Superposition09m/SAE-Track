...
def get_decode_html_safe_fn(
    tokenizer, html: bool = True
) -> Callable[[int | list[int]], str | list[str]]:
    if isinstance(tokenizer, LlamaTokenizer):
        # For LlamaTokenizer, use `_convert_id_to_token` to get the token from ID
        def decode(token_id: int | list[int]) -> str | list[str]:
            if isinstance(token_id, int):
                str_tok = tokenizer._convert_id_to_token(token_id)
                return process_str_tok(str_tok, html=html)
            else:
                if isinstance(token_id, torch.Tensor):
                    token_id = token_id.tolist()
                return [decode(tok) for tok in token_id]
    else:
        # For other tokenizers, use vocab if available
    ...
    def from_data(
        ...
        if tickmode == "ints":
            ...
            if max_value > -min_value:
                tickrange = max(0.1, round(0.1 * max_value / 3, 2))
                num_positive_ticks = 3
                num_negative_ticks = int(-min_value / tickrange)
            else:
                tickrange = max(0.1, round(0.1 * -min_value / 3, 2))
