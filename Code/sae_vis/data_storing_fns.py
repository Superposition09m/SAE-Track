...
class ActsHistogramData(HistogramData):
    def _get_html_data(
        ...
        if not self.bar_values:
            ...
            max_bar_value = max(self.bar_values)

        if max_bar_value == 0:
            bar_values_normed = [0.0] * len(self.bar_values)
            bar_colors = ["#ffffff"] * len(self.bar_values)  # Assign a default color, e.g., white
        else:
            bar_values_normed = [
                (0.4 * max_bar_value + 0.6 * v) / max_bar_value
