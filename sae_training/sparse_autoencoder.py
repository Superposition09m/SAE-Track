...
class SparseAutoencoder(HookedRootModule):
    ...
    def __init__(
        ...
        self.w_dec_norm = cfg.w_dec_norm
    ...
    def forward(self, x: torch.Tensor, dead_neuron_mask: torch.Tensor | None = None, w_dec_norm: int | None = None):
        ...
        if self.w_dec_norm == 1:
            # Original sparsity calculation
            sparsity = feature_acts.norm(p=self.lp_norm, dim=1).mean(dim=(0,))
            # print(3)
        else:
            # Normalized sparsity calculation, incorporating the decoder norm
            # print("USE NEW LOSS")
            # exit()
            decoder_norms = torch.norm(self.W_dec, dim=1, keepdim=True)#(d_sae,d_in) to (d_sae,1)
            # print("decoder_norms",decoder_norms)
            normalized_decoder = self.W_dec / (decoder_norms + 1e-6)#(d_sae,d_in)
            feature_directions = normalized_decoder * decoder_norms
            feature_activations = feature_acts * decoder_norms.squeeze()#=(... d_sae)*(d_sae,1)=(...,d_sae)
            sparsity = feature_activations.norm(p=self.lp_norm, dim=1).mean(dim=(0,))

        # Compute L1 loss with sparsity
